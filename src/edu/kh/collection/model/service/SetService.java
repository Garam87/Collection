...
public class SetService {
	...
	public void ex1() {
		...
		System.out.println( set.remove("배달의민족")); // 삭제하면 true / 못지웠음 false
		System.out.println( set.remove("아")); //  못지웠음 false
		
		System.out.println(set); // 제거 확인 용도
		
		// List는 index가 있어서 get()로 요소 하나 얻어올 수 있었지만, 
		// Set은 순서가 없어서 저장된 객체 하나를 얻어올 수 있는 방법이 없다!
		// -> 대신에 Set 전체의 데이처를 하나씩 반복적으로 얻어올 순 있다.
		
		
		// 1. Iterator(반복자)
		// - 컬렉션에서 제공하는 컬렉션 객체 반복 접근자
		// (컬렉션에 저장된 데이터를 임의로 하나씩 반복으로 꺼내는 역할)
		...
		System.out.println(set.contains("가나"));
	...
	public void ex2() {
		...
		// Hash 함수 : 입력된 단어를 지정된 길이의 문자열을 변환하는 함수(중복x)
		// ex) 입력: 111 -> "asdfg" (5글자)
		// ex) 입력: 1234569609 -> "qwert" (5글자)
		// == 암호화에 사용
		// 일정한 길이의 랜덤값 생성. 중복되지 않음
		// 암호화 하지 않으면 법적 제지받음 징역...도?
	...
	public void ex3() {
		...
		for (Member mem : memberSet) { // 해쉬코드를 오버라이딩을 해서 자동으로 중복 제거댐
	...
	public void ex4() {
		// Wrapper 클래스 정의 : 기본자료형 -> 객체로 포장하는 클래스
		...
		// 기본 자료형 -> 포장
		Integer w1 = new Integer(iNum); // 사용 권장 하지 않음(삭제선)
